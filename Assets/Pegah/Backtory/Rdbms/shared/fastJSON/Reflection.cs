...
namespace fastJSON
{
    ...
    internal sealed class Reflection
    {
        ...
        public Dictionary<string, myPropInfo> Getproperties(Type type, string typename, bool customType)
        {
            ...
            else
            {
                ...
                foreach (PropertyInfo p in pr)
                {
					...
					var displayAttributes = p.GetCustomAttributes(typeof(DisplayAttribute), true);
					var displayName = p.Name;
					if (displayAttributes.Length == 1)
						displayName = ((DisplayAttribute)displayAttributes[0]).Name;

                    myPropInfo d = CreateMyProp(p.PropertyType, p.Name, displayName, customType);
                    d.setter = Reflection.CreateSetMethod(type, p);
                    if (d.setter != null)
                        d.CanWrite = true;
                    d.getter = Reflection.CreateGetMethod(type, p);
					sd.Add(displayName.ToLower(), d);
                }
                FieldInfo[] fi = type.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static);
                foreach (FieldInfo f in fi)
                {
                    myPropInfo d = CreateMyProp(f.FieldType, f.Name, f.Name, customType);
                    if (f.IsLiteral == false)
                    {
						...
						Debug.Log("mapping " + f.Name.ToLower());
        ...
        private myPropInfo CreateMyProp(Type t, string name, string displayName, bool customType)
        {
