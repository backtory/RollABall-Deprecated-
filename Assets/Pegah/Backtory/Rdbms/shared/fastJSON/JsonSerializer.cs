        ...
        private void WriteNV(NameValueCollection nameValueCollection)
        {
            ...
            foreach (string key in nameValueCollection)
            {
                ...
                else
                {
					...
					else if (_params.SerializeToLowerCamelCaseNames){
						string camelcasekey = key.Substring(0,1).ToLower();
						if(key.Length > 1){
							camelcasekey += key.Substring(1);
						}
						WritePair(camelcasekey, nameValueCollection[key]);
					}
					else
            ...
            foreach (DictionaryEntry entry in stringDictionary)
            {
                ...
                else
                {
					...
					if (_params.SerializeToLowerCaseNames)
						...
						string camelcasekey = k.Substring(0,1).ToLower();
						if(k.Length > 1){
							camelcasekey += k.Substring(1);
						}
						WritePair(camelcasekey, entry.Value);
					}
					else
			...
			for (int ii = 0; ii < c; ii++)
            {
                ...
                else
                {
                    if (append)
                        _output.Append(',');
					if (_params.SerializeToLowerCaseNames){
						WritePair(p.Name.ToLower(), o);
					}
					else if (_params.SerializeToLowerCamelCaseNames){
						string camelcasekey = p.Name.Substring(0,1).ToLower();
						if(p.Name.Length > 1){
							camelcasekey += p.Name.Substring(1);
						}
						WritePair(camelcasekey, o);
					}
					else
						WritePair(p.Name, o);
