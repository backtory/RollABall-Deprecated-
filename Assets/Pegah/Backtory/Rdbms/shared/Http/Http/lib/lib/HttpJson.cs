...
public class HTTPJSON
{
	...
	protected static Hashtable ParseObject (char[] json, ref int index, ref bool success)
	{
		...
		while (!done) {
			...
			if (token == HTTPJSON.TOKEN_NONE) {
				success = false;
				return null;
			} else if (token == HTTPJSON.TOKEN_COMMA) {
				NextToken (json, ref index);
			} else if (token == HTTPJSON.TOKEN_CURLY_CLOSE) {
				NextToken (json, ref index);
				return table;
			} else {
	...
	protected static ArrayList ParseArray (char[] json, ref int index, ref bool success)
	{
		...
		while (!done) {
			...
			if (token == HTTPJSON.TOKEN_NONE) {
				...
				NextToken (json, ref index);
			} else if (token == HTTPJSON.TOKEN_SQUARED_CLOSE) {
				NextToken (json, ref index);
				break;
			} else {
				object value = ParseValue (json, ref index, ref success);
	...
	protected static object ParseValue (char[] json, ref int index, ref bool success)
	{
		...
		case HTTPJSON.TOKEN_STRING:
			return ParseString (json, ref index, ref success);
		case HTTPJSON.TOKEN_NUMBER:
			return ParseNumber (json, ref index, ref success);
		case HTTPJSON.TOKEN_CURLY_OPEN:
			return ParseObject (json, ref index, ref success);
		case HTTPJSON.TOKEN_SQUARED_OPEN:
			return ParseArray (json, ref index, ref success);
		case HTTPJSON.TOKEN_TRUE:
			NextToken (json, ref index);
			return true;
		case HTTPJSON.TOKEN_FALSE:
			NextToken (json, ref index);
			return false;
		case HTTPJSON.TOKEN_NULL:
			NextToken (json, ref index);
			return null;
		case HTTPJSON.TOKEN_NONE:
			break;
		}
	...
	protected static int NextToken (char[] json, ref int index)
	{
		...
		switch (c) {
		case '{':
			return HTTPJSON.TOKEN_CURLY_OPEN;
		case '}':
			return HTTPJSON.TOKEN_CURLY_CLOSE;
		case '[':
			return HTTPJSON.TOKEN_SQUARED_OPEN;
		case ']':
			return HTTPJSON.TOKEN_SQUARED_CLOSE;
		case ',':
			return HTTPJSON.TOKEN_COMMA;
		case '"':
			return HTTPJSON.TOKEN_STRING;
		case '0':
		case '1':
		case '2':
		case '3':
		...
		case '9':
		case '-':
			return HTTPJSON.TOKEN_NUMBER;
		case ':':
			return HTTPJSON.TOKEN_COLON;
		}
		index--;
